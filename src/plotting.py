...
def create_market_cap_animation(df: pd.DataFrame) -> go.Figure:
    ...
    for year in years:
        ...
        if not target_data.empty:
            ...
            target_bucket_series = pd.cut([target_market_cap], bins=market_cap_bins, labels=market_cap_labels, include_lowest=True)
            target_bucket = target_bucket_series[0] if isinstance(target_bucket_series, list) or isinstance(target_bucket_series, np.ndarray) else target_bucket_series
